...
from model import QuantizeModel
...
class CustomCallback(tf.keras.callbacks.Callback):
    def __init__(self, test_data, base_model):
        ...
        self.plot_max_length = 1024
        for i in range(min(self.x_data.shape[0], 8)):
            cv2.imwrite("./img/quantized/original_l_{}.jpg".format(i), cv2.flip(fft.minmax(
                self.y_data[i][:self.plot_max_length, :, 0].transpose(1, 0)) * 255, 0))

    def on_train_batch_begin(self, batch, logs=None):
        if batch % 250 == 0:
            pred, _ = self.base_model(self.x_data)

            for i in range(min(pred.shape[0], 8)):
                cv2.imwrite(
                    "./img/quantized/quantized_{}.jpg".format(i),
                    cv2.flip(
                        fft.minmax(
                            pred[i, :self.plot_max_length, :].numpy().transpose(
                                1,
                                0).astype("float32")) *
                        255,
                        0))
...
if __name__ == "__main__":
    ...
    ckpt_callback_best = tf.keras.callbacks.ModelCheckpoint(
        filepath="./model/quantize_model/{}.ckpt".format(model_name),
        monitor=monitor,
        verbose=1,
        save_best_only=True,
    ...
    plot_callback = CustomCallback(plot_gen, quantize_model)
